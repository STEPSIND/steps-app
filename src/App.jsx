...
function BlackHole({ onDone }) {
  ...
  useEffect(() => {
    ...
    const drawDiskHalf=(ctx,sa,ea,alpha,blur)=>{ctx.save();ctx.filter=`blur(${blur}px)`;ctx.globalAlpha=alpha;const dg=ctx.createRadialGradient(0,0,52,0,0,240);dg.addColorStop(0,'rgba(255,240,180,0.95)');dg.addColorStop(0.15,'rgba(255,180,60,0.85)');dg.addColorStop(0.35,'rgba(220,80,0,0.6)');dg.addColorStop(0.6,'rgba(140,30,0,0.3)');dg.addColorStop(0.85,'rgba(60,10,0,0.1)');dg.addColorStop(1,'rgba(0,0,0,0)');ctx.scale(1,TILT);ctx.beginPath();ctx.ellipse(0,0,240,240,0,sa,ea);ctx.lineTo(0,0);ctx.fillStyle=dg;ctx.fill();ctx.restore()}
    const draw=(now)=>{
      ...
      ctx.save();ctx.translate(cx,cy);drawDiskHalf(ctx,Math.PI,Math.PI*2,0.7,6);ctx.restore()
      ...
      ctx.save();ctx.translate(cx,cy);drawDiskHalf(ctx,0,Math.PI,1.0,4);ctx.restore()
...
function LivingBackground() {
  ...
  useEffect(()=>{
    ...
    const draw=()=>{
      ...
      for(const n of nodes){const dx=n.x-mx,dy=n.y-my,d=Math.hypot(dx,dy);if(d<130&&d>0){const f=(1-d/130)*1.1;n.vx+=(dx/d)*f;n.vy+=(dy/d)*f}const s=Math.hypot(n.vx,n.vy);if(s>2.5){n.vx=n.vx/s*2.5;n.vy=n.vy/s*2.5}n.vx*=0.94;n.vy*=0.94;n.x+=n.vx;n.y+=n.vy;n.pulse+=0.025;if(n.x<0||n.x>canvas.width)n.vx*=-1;if(n.y<0||n.y>canvas.height)n.vy*=-1}
      for(let i=0;i<nodes.length;i++)for(let j=i+1;j<nodes.length;j++){const a=nodes[i],b=nodes[j],d=Math.hypot(a.x-b.x,a.y-b.y);if(d>DIST)continue;ctx.save();ctx.globalAlpha=(1-d/DIST)*0.18;const g=ctx.createLinearGradient(a.x,a.y,b.x,b.y);g.addColorStop(0,'#E8860A');g.addColorStop(0.5,'#F5A623');g.addColorStop(1,'#7C3AED');ctx.strokeStyle=g;ctx.lineWidth=0.6;ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.stroke();ctx.restore()}
...
function DollarWidget() {
  ...
  useEffect(()=>{
    ...
    fetchRates()
    let a=0; const spin=setInterval(()=>{a=(a+0.4)%360;setCoinAngle(a)},32)
  ...
  const fetchRates=async()=>{
    setRefreshing(true)
    try{
      const[r1,r2]=await Promise.all([fetch('https://dolarapi.com/v1/dolares/oficial'),fetch('https://dolarapi.com/v1/dolares/blue')])
      const[oficial,blue]=await Promise.all([r1.json(),r2.json()])
  ...
  const applyToProducts=async()=>{
    ...
    for(const p of data||[]){
      const cost=Math.round(p.price_usd*parseFloat(lpSaved))
      const sale=p.margin?Math.round(cost*(1+p.margin/100)):null
      await supabase.from('products').update({cost_price:cost,...(sale?{sale_price:sale}:{}),cotizacion:parseFloat(lpSaved),updated_at:new Date()}).eq('id',p.id)
    }
    setApplying(false)
  }
  ...
  return (
    <div style={{position:'fixed',bottom:24,right:24,zIndex:9996}}>
      {open&&(
        <div style={{position:'absolute',bottom:58,right:0,width:272,
          ...
          boxShadow:'0 0 40px rgba(245,160,0,0.08),0 32px 80px rgba(0,0,0,0.7),inset 0 1px 0 rgba(255,255,255,0.1)',
          ...
          <div style={{padding:'10px 12px',borderRadius:10,marginBottom:10,background:'rgba(245,160,0,0.05)',border:'1px solid rgba(245,160,0,0.18)',borderTop:'1px solid rgba(245,160,0,0.3)'}}>
            ...
            <div style={{display:'flex',gap:6,alignItems:'center'}}>
              ...
              <input type="number" value={lp} onChange={e=>setLp(e.target.value)} placeholder="Ej: 1290"
                ...
                style={{flex:1,background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.09)',borderRadius:8,padding:'6px 8px',color:'#f1f5f9',fontSize:16,fontWeight:800,outline:'none'}}/>
              <button onClick={saveLp} disabled={!lp||lp===lpSaved}
                style={{padding:'6px 12px',borderRadius:8,border:'none',cursor:'pointer',background:'rgba(245,160,0,0.18)',color:'rgba(245,160,0,0.9)',fontSize:12,fontWeight:700,opacity:(!lp||lp===lpSaved)?0.35:1}}>✓</button>
            </div>
          </div>
          <button onClick={applyToProducts} disabled={!lpSaved||applying}
            style={{width:'100%',padding:'10px',borderRadius:10,border:'none',
              background:applying?'rgba(255,255,255,0.05)':lpSaved?'linear-gradient(135deg,rgba(245,160,0,0.85),rgba(220,100,0,0.75))':'rgba(255,255,255,0.04)',
              color:applying?'rgba(148,163,184,0.5)':lpSaved?'#000':'rgba(148,163,184,0.3)',
              cursor:!lpSaved||applying?'default':'pointer',fontSize:12,fontWeight:800}}>
      ...
      <button onClick={()=>setOpen(o=>!o)} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
        style={{width:48,height:48,borderRadius:'50%',border:'none',cursor:'pointer',
          ...
          transform:hov||open?'perspective(180px) rotateX(8deg) rotateY(-5deg) translateY(-4px) scale(1.12)':'perspective(180px) rotateX(14deg)',
          boxShadow:hov||open?'0 8px 0 rgba(0,0,0,0.45),0 12px 28px rgba(0,0,0,0.5),0 0 35px rgba(245,180,0,0.55),inset 0 2px 0 rgba(255,255,255,0.55)':'0 4px 0 rgba(0,0,0,0.4),0 6px 16px rgba(0,0,0,0.45),inset 0 1px 0 rgba(255,255,255,0.35)',
        ...
        <div style={{position:'absolute',width:24,height:24,borderRadius:'50%',
          ...
          left:`${shineX-12}%`,top:`${shineY-12}%`,pointerEvents:'none',mixBlendMode:'screen'}}/>
        <div style={{position:'absolute',inset:2,borderRadius:'50%',border:'1px solid rgba(255,255,255,0.15)',pointerEvents:'none'}}/>
        <span style={{position:'relative',zIndex:1,fontSize:22,fontWeight:900,
          color:hov||open?'#3D2200':'#5C3800',
          display:'flex',alignItems:'center',justifyContent:'center',
          width:'100%',height:'100%',userSelect:'none'}}>$</span>
      </button>
    </div>
  )
}
