...
function BlackHole({ onDone }) {
  ...
  useEffect(() => {
    ...
    const diskP = Array.from({ length: 280 }, () => {
      ...
      return { angle: Math.random() * Math.PI * 2, r, speed: 0.025 + Math.random() * 0.04, size: 0.8 + Math.random() * 2.2, temp: 0.4 + Math.random() * 0.6 }
    })

    const filaments = Array.from({ length: 12 }, (_, i) => ({
      angle: (i / 12) * Math.PI * 2 + Math.random() * 0.5,
      r: 180 + Math.random() * 300, speed: 0.003 + Math.random() * 0.005,
      length: 40 + Math.random() * 80, opacity: 0.2 + Math.random() * 0.5,
    }))
    ...
    const draw = (now) => {
      ...
      for (const s of stars) {
        ...
        const sy = cy + Math.sin(s.angle) * s.r * 0.55
        const fade = s.r < 60 ? s.r / 60 : 1
        ctx.save(); ctx.globalAlpha = s.brightness * fade
        ctx.fillStyle = s.color; ctx.shadowColor = s.color
        ctx.shadowBlur = s.size > 1.2 ? 4 : 0
        ctx.beginPath(); ctx.arc(sx, sy, s.size * Math.min(fade + 0.2, 1), 0, Math.PI * 2); ctx.fill(); ctx.restore()
      }
      ...
      for (let ring = 3; ring >= 0; ring--) {
        ...
        lensGrad.addColorStop(0, ringColor + '0)'); lensGrad.addColorStop(0.4, ringColor + ringAlpha + ')')
        lensGrad.addColorStop(0.65, ringColor + (ringAlpha * 2.5) + ')'); lensGrad.addColorStop(0.85, ringColor + ringAlpha + ')')
        lensGrad.addColorStop(1, ringColor + '0)'); ctx.fillStyle = lensGrad
      ...
      const drawDiskHalf = (startAngle, endAngle, alpha, blur) => {
        ...
        diskGrad.addColorStop(0, 'rgba(255,240,180,0.95)'); diskGrad.addColorStop(0.15, 'rgba(255,180,60,0.85)')
        diskGrad.addColorStop(0.35, 'rgba(220,80,0,0.6)'); diskGrad.addColorStop(0.6, 'rgba(140,30,0,0.3)')
        diskGrad.addColorStop(0.85, 'rgba(60,10,0,0.1)'); diskGrad.addColorStop(1, 'rgba(0,0,0,0)')
        ctx.scale(1, TILT); ctx.beginPath(); ctx.ellipse(0, 0, 240, 240, 0, startAngle, endAngle)
        ctx.lineTo(0, 0); ctx.fillStyle = diskGrad; ctx.fill(); ctx.restore()
      }
      drawDiskHalf(Math.PI, Math.PI * 2, 0.7, 6); ctx.restore()

      const bhR = 56
      const shadowGrad = ctx.createRadialGradient(cx, cy, bhR * 0.5, cx, cy, bhR * 2.8)
      shadowGrad.addColorStop(0, 'rgba(0,0,0,1)'); shadowGrad.addColorStop(0.38, 'rgba(0,0,0,1)')
      shadowGrad.addColorStop(0.65, 'rgba(0,0,8,0.92)'); shadowGrad.addColorStop(0.85, 'rgba(0,0,12,0.6)')
      shadowGrad.addColorStop(1, 'rgba(0,0,0,0)'); ctx.fillStyle = shadowGrad
      ctx.beginPath(); ctx.arc(cx, cy, bhR * 2.8, 0, Math.PI * 2); ctx.fill()

      const coreGrad = ctx.createRadialGradient(cx + bhR * 0.28, cy - bhR * 0.28, 2, cx, cy, bhR)
      coreGrad.addColorStop(0, '#06080f'); coreGrad.addColorStop(0.5, '#030408'); coreGrad.addColorStop(1, '#000000')
      ctx.fillStyle = coreGrad; ctx.beginPath(); ctx.arc(cx, cy, bhR, 0, Math.PI * 2); ctx.fill()

      ctx.save(); ctx.translate(cx, cy); drawDiskHalf(0, Math.PI, 1.0, 4)
      const innerGrad = ctx.createRadialGradient(0, 0, 50, 0, 0, 95)
      innerGrad.addColorStop(0, 'rgba(255,255,220,0)'); innerGrad.addColorStop(0.3, 'rgba(255,230,150,0.9)')
      innerGrad.addColorStop(0.6, 'rgba(255,150,30,0.5)'); innerGrad.addColorStop(1, 'rgba(0,0,0,0)')
      ctx.save(); ctx.scale(1, TILT); ctx.beginPath(); ctx.ellipse(0, 0, 95, 95, 0, 0, Math.PI)
      ctx.lineTo(0, 0); ctx.fillStyle = innerGrad; ctx.globalAlpha = 1.1; ctx.fill(); ctx.restore(); ctx.restore()

      for (const p of diskP) {
        p.angle += p.speed * (65 / Math.max(p.r, 8)); p.r = Math.max(0, p.r - 0.06 * (1 + progress * 3))
        if (p.r < 8) continue
        const x3d = Math.cos(p.angle) * p.r, z3d = Math.sin(p.angle) * p.r
        const px = cx + x3d, py = cy + z3d * TILT
        const distCenter = Math.hypot(px - cx, py - cy)
        if (distCenter < bhR * 0.92) continue
        const edgeFade = Math.min(1, (distCenter - bhR) / 28)
        const isFront = z3d > 0, depthAlpha = isFront ? 0.95 : 0.35
        const t = p.temp
        const pr = Math.min(255, 180 + Math.floor(t * 75)), pg = Math.floor(60 + t * 170), pb = Math.floor(t * 60)
        ctx.save(); ctx.globalAlpha = edgeFade * depthAlpha; ctx.fillStyle = `rgb(${pr},${pg},${pb})`
        if (isFront) { ctx.shadowColor = `rgb(${pr},${pg},${pb})`; ctx.shadowBlur = 4 }
        ctx.beginPath(); ctx.arc(px, py, p.size * (isFront ? 1 : 0.55), 0, Math.PI * 2); ctx.fill(); ctx.restore()
      }

      for (const f of filaments) {
        f.angle += f.speed * (1 + progress * 4); f.r = Math.max(0, f.r - f.r * 0.002 * (1 + progress * 5))
        if (f.r < 20) continue
        const fx = cx + Math.cos(f.angle) * f.r, fy = cy + Math.sin(f.angle) * f.r * 0.5
        const fx2 = cx + Math.cos(f.angle + 0.08) * (f.r - f.length), fy2 = cy + Math.sin(f.angle + 0.08) * (f.r - f.length) * 0.5
        const fade = Math.min(1, f.r / 80)
        ctx.save(); ctx.globalAlpha = f.opacity * fade; ctx.strokeStyle = 'rgba(245,160,80,0.6)'
        ctx.lineWidth = 0.8; ctx.beginPath(); ctx.moveTo(fx, fy); ctx.lineTo(fx2, fy2); ctx.stroke(); ctx.restore()
      }
      ...
      if (jetAlpha > 0.05) {
        ...
        jg1.addColorStop(0, `rgba(120,160,255,${jetAlpha})`); jg1.addColorStop(0.3, `rgba(90,120,230,${jetAlpha * 0.7})`)
        jg1.addColorStop(0.7, `rgba(70,90,200,${jetAlpha * 0.3})`); jg1.addColorStop(1, 'rgba(60,80,200,0)')
        ctx.save(); ctx.fillStyle = jg1; ctx.shadowColor = 'rgba(120,160,255,0.4)'; ctx.shadowBlur = 12
        ctx.beginPath(); ctx.moveTo(cx - 5, cy - bhR); ctx.lineTo(cx + 5, cy - bhR)
        ctx.lineTo(cx + 1.5, cy - bhR - jetH); ctx.lineTo(cx - 1.5, cy - bhR - jetH); ctx.fill(); ctx.restore()
        const jg2 = ctx.createLinearGradient(cx, cy + bhR, cx, cy + bhR + jetH)
        jg2.addColorStop(0, `rgba(120,160,255,${jetAlpha})`); jg2.addColorStop(0.3, `rgba(90,120,230,${jetAlpha * 0.7})`)
        jg2.addColorStop(1, 'rgba(60,80,200,0)')
        ctx.save(); ctx.fillStyle = jg2; ctx.shadowColor = 'rgba(120,160,255,0.3)'; ctx.shadowBlur = 10
        ctx.beginPath(); ctx.moveTo(cx - 5, cy + bhR); ctx.lineTo(cx + 5, cy + bhR)
        ctx.lineTo(cx + 1.5, cy + bhR + jetH); ctx.lineTo(cx - 1.5, cy + bhR + jetH); ctx.fill(); ctx.restore()
      }
      ...
      if (progress > 0.72) {
        ...
        if (ep < 0.6) {
          ...
          edgeGrad.addColorStop(0, 'rgba(0,0,0,0)'); edgeGrad.addColorStop(0.5, `rgba(245,160,0,${ep * 0.4})`)
          edgeGrad.addColorStop(1, 'rgba(0,0,0,0)'); ctx.fillStyle = edgeGrad
          ctx.beginPath(); ctx.arc(cx, cy, expandR, 0, Math.PI * 2); ctx.fill()
        }
        ctx.fillStyle = '#000000'; ctx.globalAlpha = Math.pow(ep, 1.4)
        ctx.beginPath(); ctx.arc(cx, cy, expandR * 0.96, 0, Math.PI * 2); ctx.fill(); ctx.globalAlpha = 1
      }

      if (elapsed < DURATION) frameRef.current = requestAnimationFrame(draw)
      else onDone()
    }
...
function LivingBackground() {
  ...
  useEffect(() => {
    ...
    const nodes = Array.from({length: 80}, () => ({
      ...
      r: 1.2 + Math.random() * 1.8, pulse: Math.random() * Math.PI * 2,
    }))
    const CONNECT_DIST = 140

    const draw = () => {
      ctx.clearRect(0, 0, canvas.width, canvas.height)
      const mx = mouseRef.current?.x ?? -1000, my = mouseRef.current?.y ?? -1000
      for (const n of nodes) {
        const dx = n.x - mx, dy = n.y - my, dist = Math.hypot(dx, dy), REPEL = 130
        if (dist < REPEL && dist > 0) { const force = (1 - dist / REPEL) * 1.1; n.vx += (dx / dist) * force; n.vy += (dy / dist) * force }
        const spd = Math.hypot(n.vx, n.vy)
        if (spd > 2.5) { n.vx = (n.vx/spd)*2.5; n.vy = (n.vy/spd)*2.5 }
        n.vx *= 0.94; n.vy *= 0.94; n.x += n.vx; n.y += n.vy; n.pulse += 0.025
      ...
      for (let i = 0; i < nodes.length; i++) {
        for (let j = i + 1; j < nodes.length; j++) {
          ...
          grad.addColorStop(0, '#E8860A'); grad.addColorStop(0.5, '#F5A623'); grad.addColorStop(1, '#7C3AED')
      ...
      for (const n of nodes) {
        ...
        g.addColorStop(0, '#F5A623'); g.addColorStop(0.5, 'rgba(232,134,10,0.3)'); g.addColorStop(1, 'transparent')
        ctx.fillStyle = g; ctx.beginPath(); ctx.arc(n.x, n.y, n.r * 3, 0, Math.PI * 2); ctx.fill()
        ctx.globalAlpha = pulse; ctx.fillStyle = '#F5A623'
        ctx.beginPath(); ctx.arc(n.x, n.y, n.r, 0, Math.PI * 2); ctx.fill(); ctx.restore()
      }
      frameRef.current = requestAnimationFrame(draw)
    }
    draw()
    return () => { window.removeEventListener('resize', resize); window.removeEventListener('mousemove', onMouseMove); cancelAnimationFrame(frameRef.current) }
  }, [])
...
function DollarWidget() {
  ...
  const applyToProducts = async () => {
    ...
    for (const p of data || []) {
      ...
      await supabase.from('products').update({ cost_price: cost, ...(sale ? { sale_price: sale } : {}), cotizacion: +lpSaved, updated_at: new Date() }).eq('id', p.id)
    }
    setApplying(false)
  }
    ...
    <div style={{ position:'fixed', bottom:24, right:24, zIndex:9996 }}>
      {open && (
        <div style={{
          ...
          background:'rgba(3,3,14,0.94)', backdropFilter:'blur(48px) saturate(200%)',
          WebkitBackdropFilter:'blur(48px) saturate(200%)',
          border:'1px solid rgba(255,255,255,0.09)', borderTop:'1px solid rgba(255,255,255,0.2)',
          borderRadius:18, padding:'16px 16px 14px',
          boxShadow:'0 0 0 1px rgba(245,160,0,0.08),0 0 40px rgba(245,160,0,0.08),0 32px 80px rgba(0,0,0,0.7),inset 0 1px 0 rgba(255,255,255,0.1)',
          ...
          <div style={{padding:'10px 12px',borderRadius:10,marginBottom:10,background:'rgba(245,160,0,0.05)',border:'1px solid rgba(245,160,0,0.18)',borderTop:'1px solid rgba(245,160,0,0.3)'}}>
            ...
            <div style={{display:'flex',gap:6,alignItems:'center'}}>
              ...
              <input type="number" value={lp} onChange={e=>setLp(e.target.value)} placeholder="Ej: 1290"
                onKeyDown={e=>e.key==='Enter'&&saveLp()}
                style={{flex:1,background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.09)',borderRadius:8,padding:'6px 8px',color:'#f1f5f9',fontSize:16,fontWeight:800,outline:'none',letterSpacing:'-0.02em'}}/>
              <button onClick={saveLp} disabled={!lp||lp===lpSaved}
                style={{padding:'6px 12px',borderRadius:8,border:'none',cursor:'pointer',background:'rgba(245,160,0,0.18)',color:'rgba(245,160,0,0.9)',fontSize:12,fontWeight:700,flexShrink:0,opacity:(!lp||lp===lpSaved)?0.35:1,transition:'opacity 0.2s'}}>✓</button>
            </div>
          </div>
          <button onClick={applyToProducts} disabled={!lpSaved||applying}
            style={{width:'100%',padding:'10px',borderRadius:10,border:'none',background:applying?'rgba(255,255,255,0.05)':lpSaved?'linear-gradient(135deg,rgba(245,160,0,0.85),rgba(220,100,0,0.75))':'rgba(255,255,255,0.04)',color:applying?'rgba(148,163,184,0.5)':lpSaved?'#000':'rgba(148,163,184,0.3)',cursor:!lpSaved||applying?'default':'pointer',fontSize:12,fontWeight:800,boxShadow:lpSaved&&!applying?'0 0 24px rgba(245,160,0,0.2)':'none',transition:'all 0.2s',letterSpacing:'0.01em'}}>
      ...
      <button onClick={()=>setOpen(o=>!o)} onMouseEnter={()=>setHovered(true)} onMouseLeave={()=>setHovered(false)}
        style={{width:48,height:48,borderRadius:'50%',border:'none',cursor:'pointer',position:'relative',overflow:'hidden',
          background:`conic-gradient(from ${coinAngle}deg,#C8941A 0%,#F5D060 15%,#FFE87C 22%,#D4A520 30%,#8B6510 40%,#C8941A 50%,#F5D060 65%,#FFE87C 72%,#D4A520 80%,#8B6510 90%,#C8941A 100%)`,
          transform:hovered||open?'perspective(180px) rotateX(8deg) rotateY(-5deg) translateY(-4px) scale(1.12)':'perspective(180px) rotateX(14deg) rotateY(0deg)',
          boxShadow:hovered||open?'0 8px 0 rgba(0,0,0,0.45),0 12px 28px rgba(0,0,0,0.5),0 0 0 1px rgba(245,200,80,0.5),0 0 35px rgba(245,180,0,0.55),inset 0 2px 0 rgba(255,255,255,0.55),inset 0 -2px 0 rgba(0,0,0,0.3)':'0 4px 0 rgba(0,0,0,0.4),0 6px 16px rgba(0,0,0,0.45),inset 0 1px 0 rgba(255,255,255,0.35),inset 0 -1px 0 rgba(0,0,0,0.25)',
          transition:'transform 0.28s cubic-bezier(0.34,1.3,0.64,1),box-shadow 0.28s ease'}}>
        <div style={{position:'absolute',width:24,height:24,borderRadius:'50%',background:'radial-gradient(circle,rgba(255,255,255,0.7) 0%,rgba(255,255,255,0) 70%)',left:`${shineX-12}%`,top:`${shineY-12}%`,transform:'scale(0.85)',pointerEvents:'none',mixBlendMode:'screen',transition:'left 0.1s,top 0.1s'}}/>
        <div style={{position:'absolute',inset:2,borderRadius:'50%',border:'1px solid rgba(255,255,255,0.15)',pointerEvents:'none'}}/>
        <span style={{position:'relative',zIndex:1,fontSize:22,fontWeight:900,lineHeight:1,color:hovered||open?'#3D2200':'#5C3800',textShadow:'0 1px 0 rgba(255,255,255,0.45),0 -1px 0 rgba(0,0,0,0.35)',display:'flex',alignItems:'center',justifyContent:'center',width:'100%',height:'100%',letterSpacing:'-1px',transition:'color 0.2s',userSelect:'none'}}>$</span>
      </button>
    </div>
  )
}
...
const NAV_ICONS = {
  ...
  '/ventas': (<svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="2,14 7,8 11,11 18,4"/><polyline points="14,4 18,4 18,8"/></svg>),
...
function useRippleEffect() {
  useEffect(() => {
    const handler = (e) => {
      ...
      target.classList.add('ripple-origin'); target.appendChild(wave)
  ...
  useEffect(() => {
    const update = (e) => {
      document.querySelectorAll('.spotlight-card').forEach(card => {
        ...
        card.style.setProperty('--sx', `${x}%`); card.style.setProperty('--sy', `${y}%`)
          ...
          {!collapsed&&(
            <div style={{padding:'10px 14px',borderTop:'1px solid rgba(255,255,255,0.05)',flexShrink:0}}>
              ...
              <button onClick={logout}
                style={{width:'100%',padding:'7px',borderRadius:7,border:'1px solid rgba(255,255,255,0.07)',background:'transparent',color:'#475569',cursor:'pointer',fontSize:11,transition:'all .2s',fontFamily:'var(--font-body)'}}
