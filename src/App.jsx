        ...
        {toasts.map(t => {
          ...
          return (
            <div key={t.id} style={{
              ...
              animation:'toastIn 0.35s cubic-bezier(0.34,1.56,0.64,1) forwards',fontFamily:'system-ui',
            }}>
        ...
        <>
          ...
          {!collapsed && (
            <span style={{fontSize:12,fontWeight:isActive?600:400,
              ...
              fontFamily:'var(--font-body)',whiteSpace:'nowrap',overflow:'hidden',transition:'color 0.15s'}}>
              {n.label}
            </span>
          )}
          {collapsed && (
            <div style={{position:'absolute',left:52,top:'50%',transform:'translateY(-50%)',
              background:'rgba(8,4,20,0.96)',border:'1px solid rgba(255,255,255,0.12)',
              borderRadius:8,padding:'5px 10px',fontSize:11,fontWeight:600,color:'#f1f5f9',
              whiteSpace:'nowrap',pointerEvents:'none',opacity:0,zIndex:200,
              boxShadow:'0 4px 16px rgba(0,0,0,0.5)',fontFamily:'var(--font-body)',
...
function BlackHole({ onDone }) {
  ...
  useEffect(() => {
    ...
    const diskP = Array.from({length:280},()=>{const r=58+Math.random()*200;return{angle:Math.random()*Math.PI*2,r,speed:0.025+Math.random()*0.04,size:0.8+Math.random()*2.2,temp:0.4+Math.random()*0.6}})
    ...
    const draw = (now) => {
      ...
      for(let ring=3;ring>=0;ring--){const ringR=62+ring*55,ringAlpha=[0.06,0.1,0.14,0.07][ring],ringColor=ring<2?'rgba(255,210,100,':'rgba(245,140,0,',lg=ctx.createRadialGradient(cx,cy,ringR*0.7,cx,cy,ringR*1.3);lg.addColorStop(0,ringColor+'0)');lg.addColorStop(0.4,ringColor+ringAlpha+')');lg.addColorStop(0.65,ringColor+(ringAlpha*2.5)+')');lg.addColorStop(0.85,ringColor+ringAlpha+')');lg.addColorStop(1,ringColor+'0)');ctx.fillStyle=lg;ctx.beginPath();ctx.arc(cx,cy,ringR*1.3,0,Math.PI*2);ctx.fill()}
      ctx.save();ctx.translate(cx,cy)
      const drawDiskHalf=(sa,ea,alpha,blur)=>{ctx.save();ctx.filter=`blur(${blur}px)`;ctx.globalAlpha=alpha;const dg=ctx.createRadialGradient(0,0,52,0,0,240);dg.addColorStop(0,'rgba(255,240,180,0.95)');dg.addColorStop(0.15,'rgba(255,180,60,0.85)');dg.addColorStop(0.35,'rgba(220,80,0,0.6)');dg.addColorStop(0.6,'rgba(140,30,0,0.3)');dg.addColorStop(0.85,'rgba(60,10,0,0.1)');dg.addColorStop(1,'rgba(0,0,0,0)');ctx.scale(1,TILT);ctx.beginPath();ctx.ellipse(0,0,240,240,0,sa,ea);ctx.lineTo(0,0);ctx.fillStyle=dg;ctx.fill();ctx.restore()}
      drawDiskHalf(Math.PI,Math.PI*2,0.7,6);ctx.restore()
      ...
      ctx.save();ctx.translate(cx,cy);drawDiskHalf(0,Math.PI,1.0,4)
      const ig=ctx.createRadialGradient(0,0,50,0,0,95);ig.addColorStop(0,'rgba(255,255,220,0)');ig.addColorStop(0.3,'rgba(255,230,150,0.9)');ig.addColorStop(0.6,'rgba(255,150,30,0.5)');ig.addColorStop(1,'rgba(0,0,0,0)');ctx.save();ctx.scale(1,TILT);ctx.beginPath();ctx.ellipse(0,0,95,95,0,0,Math.PI);ctx.lineTo(0,0);ctx.fillStyle=ig;ctx.globalAlpha=1.1;ctx.fill();ctx.restore();ctx.restore()
      for(const p of diskP){p.angle+=p.speed*(65/Math.max(p.r,8));p.r=Math.max(0,p.r-0.06*(1+progress*3));if(p.r<8)continue;const x3d=Math.cos(p.angle)*p.r,z3d=Math.sin(p.angle)*p.r,px=cx+x3d,py=cy+z3d*TILT,dc=Math.hypot(px-cx,py-cy);if(dc<bhR*0.92)continue;const ef=Math.min(1,(dc-bhR)/28),isFront=z3d>0,da=isFront?0.95:0.35,t=p.temp,pr=Math.min(255,180+Math.floor(t*75)),pg=Math.floor(60+t*170),pb=Math.floor(t*60);ctx.save();ctx.globalAlpha=ef*da;ctx.fillStyle=`rgb(${pr},${pg},${pb})`;if(isFront){ctx.shadowColor=`rgb(${pr},${pg},${pb})`;ctx.shadowBlur=4}ctx.beginPath();ctx.arc(px,py,p.size*(isFront?1:0.55),0,Math.PI*2);ctx.fill();ctx.restore()}
      for(const f of filaments){f.angle+=f.speed*(1+progress*4);f.r=Math.max(0,f.r-f.r*0.002*(1+progress*5));if(f.r<20)continue;const fx=cx+Math.cos(f.angle)*f.r,fy=cy+Math.sin(f.angle)*f.r*0.5,fx2=cx+Math.cos(f.angle+0.08)*(f.r-f.length),fy2=cy+Math.sin(f.angle+0.08)*(f.r-f.length)*0.5,fade=Math.min(1,f.r/80);ctx.save();ctx.globalAlpha=f.opacity*fade;ctx.strokeStyle='rgba(245,160,80,0.6)';ctx.lineWidth=0.8;ctx.beginPath();ctx.moveTo(fx,fy);ctx.lineTo(fx2,fy2);ctx.stroke();ctx.restore()}
      const ja=Math.min(progress*1.5,0.55);if(ja>0.05){const jH=340,jg1=ctx.createLinearGradient(cx,cy-bhR,cx,cy-bhR-jH);jg1.addColorStop(0,`rgba(120,160,255,${ja})`);jg1.addColorStop(0.3,`rgba(90,120,230,${ja*0.7})`);jg1.addColorStop(0.7,`rgba(70,90,200,${ja*0.3})`);jg1.addColorStop(1,'rgba(60,80,200,0)');ctx.save();ctx.fillStyle=jg1;ctx.shadowColor='rgba(120,160,255,0.4)';ctx.shadowBlur=12;ctx.beginPath();ctx.moveTo(cx-5,cy-bhR);ctx.lineTo(cx+5,cy-bhR);ctx.lineTo(cx+1.5,cy-bhR-jH);ctx.lineTo(cx-1.5,cy-bhR-jH);ctx.fill();ctx.restore();const jg2=ctx.createLinearGradient(cx,cy+bhR,cx,cy+bhR+jH);jg2.addColorStop(0,`rgba(120,160,255,${ja})`);jg2.addColorStop(0.3,`rgba(90,120,230,${ja*0.7})`);jg2.addColorStop(1,'rgba(60,80,200,0)');ctx.save();ctx.fillStyle=jg2;ctx.shadowColor='rgba(120,160,255,0.3)';ctx.shadowBlur=10;ctx.beginPath();ctx.moveTo(cx-5,cy+bhR);ctx.lineTo(cx+5,cy+bhR);ctx.lineTo(cx+1.5,cy+bhR+jH);ctx.lineTo(cx-1.5,cy+bhR+jH);ctx.fill();ctx.restore()}
      if(progress>0.72){const ep=(progress-0.72)/0.28,er=bhR+Math.pow(ep,1.8)*Math.max(W,H)*2.2;if(ep<0.6){const eg=ctx.createRadialGradient(cx,cy,er*0.94,cx,cy,er);eg.addColorStop(0,'rgba(0,0,0,0)');eg.addColorStop(0.5,`rgba(245,160,0,${ep*0.4})`);eg.addColorStop(1,'rgba(0,0,0,0)');ctx.fillStyle=eg;ctx.beginPath();ctx.arc(cx,cy,er,0,Math.PI*2);ctx.fill()}ctx.fillStyle='#000000';ctx.globalAlpha=Math.pow(ep,1.4);ctx.beginPath();ctx.arc(cx,cy,er*0.96,0,Math.PI*2);ctx.fill();ctx.globalAlpha=1}
...
function LivingBackground() {
  ...
  useEffect(()=>{
    ...
    const draw=()=>{
      ...
      for(const n of nodes){const dx=n.x-mx,dy=n.y-my,dist=Math.hypot(dx,dy),REPEL=130;if(dist<REPEL&&dist>0){const force=(1-dist/REPEL)*1.1;n.vx+=(dx/dist)*force;n.vy+=(dy/dist)*force}const spd=Math.hypot(n.vx,n.vy);if(spd>2.5){n.vx=(n.vx/spd)*2.5;n.vy=(n.vy/spd)*2.5}n.vx*=0.94;n.vy*=0.94;n.x+=n.vx;n.y+=n.vy;n.pulse+=0.025;if(n.x<0||n.x>canvas.width)n.vx*=-1;if(n.y<0||n.y>canvas.height)n.vy*=-1}
      for(let i=0;i<nodes.length;i++){for(let j=i+1;j<nodes.length;j++){const a=nodes[i],b=nodes[j],dist=Math.hypot(a.x-b.x,a.y-b.y);if(dist>CONNECT_DIST)continue;const alpha=(1-dist/CONNECT_DIST)*0.18;ctx.save();ctx.globalAlpha=alpha;const g=ctx.createLinearGradient(a.x,a.y,b.x,b.y);g.addColorStop(0,'#E8860A');g.addColorStop(0.5,'#F5A623');g.addColorStop(1,'#7C3AED');ctx.strokeStyle=g;ctx.lineWidth=0.6;ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.stroke();ctx.restore()}}
      for(const n of nodes){const pulse=Math.sin(n.pulse)*0.4+0.6;ctx.save();ctx.globalAlpha=pulse*0.7;const g=ctx.createRadialGradient(n.x,n.y,0,n.x,n.y,n.r*3);g.addColorStop(0,'#F5A623');g.addColorStop(0.5,'rgba(232,134,10,0.3)');g.addColorStop(1,'transparent');ctx.fillStyle=g;ctx.beginPath();ctx.arc(n.x,n.y,n.r*3,0,Math.PI*2);ctx.fill();ctx.globalAlpha=pulse;ctx.fillStyle='#F5A623';ctx.beginPath();ctx.arc(n.x,n.y,n.r,0,Math.PI*2);ctx.fill();ctx.restore()}
      frameRef.current=requestAnimationFrame(draw)
    }
    draw()
...
function DollarWidget() {
  ...
  useEffect(()=>{
    ...
    fetchRates()
    let angle=0
    const spin=setInterval(()=>{angle=(angle+0.4)%360;setCoinAngle(angle)},32)
  ...
  const fetchRates=async()=>{
    setRefreshing(true)
    try{
      const[ofRes,blRes]=await Promise.all([fetch('https://dolarapi.com/v1/dolares/oficial'),fetch('https://dolarapi.com/v1/dolares/blue')])
      const[oficial,blue]=await Promise.all([ofRes.json(),blRes.json()])
      setRates(prev=>{setPrevRates({oficial:prev.oficial?.venta,blue:prev.blue?.venta});return{oficial,blue}})
  ...
  const arrow=(current,prev)=>{if(!prev||!current)return{sym:'—',col:'rgba(148,163,184,0.4)'};if(+current>+prev)return{sym:'↑',col:'#84cc16'};if(+current<+prev)return{sym:'↓',col:'#f43f5e'};return{sym:'→',col:'rgba(148,163,184,0.5)'}}
      ...
      {open&&(
        <div style={{position:'absolute',bottom:58,right:0,width:272,background:'rgba(3,3,14,0.94)',backdropFilter:'blur(48px) saturate(200%)',WebkitBackdropFilter:'blur(48px) saturate(200%)',border:'1px solid rgba(255,255,255,0.09)',borderTop:'1px solid rgba(255,255,255,0.2)',borderRadius:18,padding:'16px 16px 14px',boxShadow:'0 0 0 1px rgba(245,160,0,0.08),0 0 40px rgba(245,160,0,0.08),0 32px 80px rgba(0,0,0,0.7),inset 0 1px 0 rgba(255,255,255,0.1)',animation:'dollarPop 0.32s cubic-bezier(0.34,1.3,0.64,1)'}}>
          ...
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14}}>
            ...
            <button onClick={fetchRates} disabled={refreshing} style={{background:'none',border:'none',color:'rgba(148,163,184,0.45)',cursor:'pointer',fontSize:14,transform:refreshing?'rotate(180deg)':'rotate(0)',transition:'transform 0.5s',padding:'2px 6px'}}>↻</button>
          </div>
            ...
            return(
              <div key={key} style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'9px 12px',borderRadius:10,marginBottom:7,background:'rgba(255,255,255,0.035)',border:'1px solid rgba(255,255,255,0.06)',borderTop:'1px solid rgba(255,255,255,0.1)'}}>
                ...
                <div style={{display:'flex',alignItems:'center',gap:10}}>
                  ...
                  <span style={{fontSize:15,fontWeight:800,color:'#f1f5f9',letterSpacing:'-0.02em'}}>{fmt(data?.venta)}</span>
          ...
          <div style={{padding:'10px 12px',borderRadius:10,marginBottom:10,background:'rgba(245,160,0,0.05)',border:'1px solid rgba(245,160,0,0.18)',borderTop:'1px solid rgba(245,160,0,0.3)'}}>
            ...
            <div style={{display:'flex',gap:6,alignItems:'center'}}>
              ...
              <input type="number" value={lp} onChange={e=>setLp(e.target.value)} placeholder="Ej: 1290" onKeyDown={e=>e.key==='Enter'&&saveLp()} style={{flex:1,background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.09)',borderRadius:8,padding:'6px 8px',color:'#f1f5f9',fontSize:16,fontWeight:800,outline:'none',letterSpacing:'-0.02em'}}/>
              <button onClick={saveLp} disabled={!lp||lp===lpSaved} style={{padding:'6px 12px',borderRadius:8,border:'none',cursor:'pointer',background:'rgba(245,160,0,0.18)',color:'rgba(245,160,0,0.9)',fontSize:12,fontWeight:700,flexShrink:0,opacity:(!lp||lp===lpSaved)?0.35:1,transition:'opacity 0.2s'}}>✓</button>
            </div>
          </div>
          <button onClick={applyToProducts} disabled={!lpSaved||applying} style={{width:'100%',padding:'10px',borderRadius:10,border:'none',background:applying?'rgba(255,255,255,0.05)':lpSaved?'linear-gradient(135deg,rgba(245,160,0,0.85),rgba(220,100,0,0.75))':'rgba(255,255,255,0.04)',color:applying?'rgba(148,163,184,0.5)':lpSaved?'#000':'rgba(148,163,184,0.3)',cursor:!lpSaved||applying?'default':'pointer',fontSize:12,fontWeight:800,boxShadow:lpSaved&&!applying?'0 0 24px rgba(245,160,0,0.2)':'none',transition:'all 0.2s',letterSpacing:'0.01em'}}>
            {applying?'⏳ Actualizando...':lpSaved?`⚡ Aplicar $${lpSaved} a productos`:'Definí el LP primero'}
          </button>
        </div>
      )}
      <button onClick={()=>setOpen(o=>!o)} onMouseEnter={()=>setHovered(true)} onMouseLeave={()=>setHovered(false)}
        style={{width:48,height:48,borderRadius:'50%',border:'none',cursor:'pointer',position:'relative',overflow:'hidden',background:`conic-gradient(from ${coinAngle}deg,#C8941A 0%,#F5D060 15%,#FFE87C 22%,#D4A520 30%,#8B6510 40%,#C8941A 50%,#F5D060 65%,#FFE87C 72%,#D4A520 80%,#8B6510 90%,#C8941A 100%)`,transform:hovered||open?'perspective(180px) rotateX(8deg) rotateY(-5deg) translateY(-4px) scale(1.12)':'perspective(180px) rotateX(14deg) rotateY(0deg)',boxShadow:hovered||open?'0 8px 0 rgba(0,0,0,0.45),0 12px 28px rgba(0,0,0,0.5),0 0 0 1px rgba(245,200,80,0.5),0 0 35px rgba(245,180,0,0.55),inset 0 2px 0 rgba(255,255,255,0.55),inset 0 -2px 0 rgba(0,0,0,0.3)':'0 4px 0 rgba(0,0,0,0.4),0 6px 16px rgba(0,0,0,0.45),inset 0 1px 0 rgba(255,255,255,0.35),inset 0 -1px 0 rgba(0,0,0,0.25)',transition:'transform 0.28s cubic-bezier(0.34,1.3,0.64,1),box-shadow 0.28s ease'}}>
        <div style={{position:'absolute',width:24,height:24,borderRadius:'50%',background:'radial-gradient(circle,rgba(255,255,255,0.7) 0%,rgba(255,255,255,0) 70%)',left:`${shineX-12}%`,top:`${shineY-12}%`,transform:'scale(0.85)',pointerEvents:'none',mixBlendMode:'screen'}}/>
        <div style={{position:'absolute',inset:2,borderRadius:'50%',border:'1px solid rgba(255,255,255,0.15)',pointerEvents:'none'}}/>
        <span style={{position:'relative',zIndex:1,fontSize:22,fontWeight:900,lineHeight:1,color:hovered||open?'#3D2200':'#5C3800',textShadow:'0 1px 0 rgba(255,255,255,0.45),0 -1px 0 rgba(0,0,0,0.35)',display:'flex',alignItems:'center',justifyContent:'center',width:'100%',height:'100%',letterSpacing:'-1px',userSelect:'none'}}>$</span>
      </button>
    </div>
  )
}
...
const NAV_ICONS = {
  ...
  '/ventas':         (<svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M3 3h14l-1.5 9H4.5z"/><circle cx="7" cy="17" r="1.2"/><circle cx="13" cy="17" r="1.2"/><path d="M9 9V7m2 2V6"/></svg>),
  ...
  '/carga-productos':(<svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M10 2l1.5 3.5L15 6.5l-2.5 2.5.5 3.5L10 11l-3 1.5.5-3.5L5 6.5l3.5-1z"/><path d="M4 16l2-2m10 2l-2-2" opacity="0.5"/><line x1="10" y1="13" x2="10" y2="18" opacity="0.5"/></svg>),
  ...
  '/compras':        (<svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M3 4h14l-1.5 8H4.5L3 4z"/><path d="M3 4L2 2H1"/><circle cx="7" cy="17" r="1"/><circle cx="13" cy="17" r="1"/><line x1="9" y1="8" x2="9" y2="12"/><line x1="7" y1="10" x2="11" y2="10"/></svg>),
  '/balance':        (<svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><line x1="10" y1="2" x2="10" y2="18"/><path d="M6 5l-4 5h8z"/><path d="M14 10l-4 5h8z" opacity="0.7"/><line x1="4" y1="17" x2="8" y2="17"/><line x1="12" y1="17" x2="16" y2="17"/></svg>),
  '/caja':           (<svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="5" width="16" height="12" rx="2"/><path d="M2 9h16"/><circle cx="13" cy="14" r="1" fill="currentColor" stroke="none"/><line x1="5" y1="14" x2="9" y2="14"/></svg>),
  ...
  return (
    <div onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      style={{width:32,height:32,borderRadius:9,flexShrink:0,display:'flex',alignItems:'center',justifyContent:'center',
        ...
        transform:hov?'perspective(400px) rotateX(-12deg) rotateY(10deg) translateY(-2px)':active?'perspective(400px) rotateX(-5deg) rotateY(4deg)':'perspective(400px) rotateX(0deg)',
...
function App() {
  ...
  if(loading) return (
    <div style={{minHeight:'100vh',background:'#07070f',display:'flex',alignItems:'center',justifyContent:'center',color:'#06b6d4',fontFamily:'system-ui'}}>
      <div style={{textAlign:'center'}}><div style={{fontSize:40,marginBottom:8}}>⚡</div><div style={{fontSize:12,color:'#475569'}}>Iniciando sistema...</div></div>
    </div>
  )
      ...
      <div style={{display:'flex',minHeight:'100vh',background:c.bg,color:c.text,position:'relative',fontFamily:'system-ui,-apple-system,sans-serif'}}>
        ...
        <div style={{width:collapsed?60:210,flexShrink:0,display:'flex',flexDirection:'column',position:'sticky',top:0,height:'100vh',overflowY:'auto',overflowX:'hidden',background:'linear-gradient(180deg,rgba(8,4,20,0.99),rgba(6,3,16,0.98))',borderRight:'1px solid rgba(255,255,255,0.05)',zIndex:100,transition:'width 0.3s cubic-bezier(0.4,0,0.2,1)'}}>
          ...
          <div style={{padding:collapsed?'18px 0':'18px 16px 14px',borderBottom:'1px solid rgba(255,255,255,0.05)',flexShrink:0,display:'flex',alignItems:'center',justifyContent:collapsed?'center':'flex-start',gap:10,overflow:'hidden',position:'relative'}}>
            ...
            <img src="/logo.png" alt="STEPS" style={{height:collapsed?22:26,width:'auto',flexShrink:0,filter:'brightness(1.1) drop-shadow(0 0 10px rgba(232,134,10,0.5))',transition:'all 0.3s ease'}} onError={e=>e.target.style.display='none'}/>
            {!collapsed&&<div style={{fontSize:8,color:'rgba(148,163,184,0.4)',fontFamily:'var(--font-mono)',textTransform:'uppercase',letterSpacing:'0.18em',whiteSpace:'nowrap'}}>Command Center</div>}
          </div>
