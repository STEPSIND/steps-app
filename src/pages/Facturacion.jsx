...
function KpiCard({ value, label, color, sub, icon }) {
  ...
  return (
    <div ref={ref} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)} onMouseMove={onMove}
      ...
      <div style={{fontSize:11,color:w.muted,marginTop:4,fontWeight:600,letterSpacing:'0.02em'}}>{label}</div>
...
function InvoiceRow({ inv, onClick }) {
  ...
  return (
    <div onClick={onClick} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      style={{
        display:'grid', gridTemplateColumns:'80px 1fr 140px 120px 110px 110px 90px',
      ...
      <div style={{fontFamily:"'Space Mono',monospace",fontSize:12,fontWeight:700,color:w.orange}}>
        F{inv.tipo} {String(inv.number).padStart(6,'0')}
      </div>
      {/* Cliente */}
      <div>
      ...
      {/* Pago */}
      <div style={{fontSize:10,color:w.muted,textAlign:'center',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>
...
// ── PAYMENT ITEM ──────────────────────────────────────────────────────────────
function PaymentRow({ pay, onRemove }) {
  const PAY_STATUS = { PENDIENTE: w.amber, COBRADO: w.lime, VENCIDO: w.rose }
  const color = PAY_STATUS[pay.status] || w.muted
  return (
    <div style={{display:'flex',gap:10,alignItems:'center',padding:'10px 14px',
      ...
      <div style={{flex:1,minWidth:120}}>
        <div style={{fontSize:13,fontWeight:700,color:w.text}}>{pay.method}</div>
        {pay.due_date && <div style={{fontSize:11,color:w.muted}}>Vence: {fmtDate(pay.due_date)}</div>}
        {pay.notes && <div style={{fontSize:11,color:w.sub,fontStyle:'italic'}}>{pay.notes}</div>}
      </div>
      <div style={{fontFamily:"'Space Mono',monospace",fontSize:13,fontWeight:800,color:w.orange}}>
        {fmtMoney(pay.amount)}
      </div>
      <span style={{fontSize:10,fontWeight:700,padding:'3px 8px',borderRadius:20,
        background:color+'18',color,border:`1px solid ${color}33`}}>{pay.status}</span>
...
function InvoiceModal({ initial, clients, quotes, onClose, onSaved }) {
  ...
  useEffect(() => {
    ...
    const iva21 = items.filter(it=>it.iva_pct===21||it.iva_pct==='21').reduce((s,it)=>s+(it.subtotal||0)*0.21,0)
    const iva105 = items.filter(it=>it.iva_pct===10.5||it.iva_pct==='10.5').reduce((s,it)=>s+(it.subtotal||0)*0.105,0)
  ...
  const calcItem = (it) => {
    ...
    const ivaAmt = round(sub * ((it.iva_pct||21)/100))
    return { ...it, subtotal: sub, subtotal_with_iva: round(sub + ivaAmt) }
  }
  ...
  const updateItem = (idx, key, val) => {
    setItems(its => its.map((it,i) => i===idx ? calcItem({...it,[key]:key==='quantity'||key==='unit_price'||key==='discount_pct'||key==='iva_pct'?Number(val):val}) : it))
  }
  ...
  const save = async () => {
    ...
    ;['neto','iva_21','iva_105','otros_tributos','total','number'].forEach(k => {
      ...
      <div onClick={e=>e.stopPropagation()} style={{
        ...
        <div style={{flex:1,overflowY:'auto',padding:'20px 26px',display:'flex',flexDirection:'column',gap:14}}>
          ...
          {tab==='datos' && (
            <>
                    ...
                    {TIPOS.map(t=>(
                      <button key={t} onClick={()=>set('tipo',t)}
                        style={{flex:1,padding:'10px 0',borderRadius:10,cursor:'pointer',border:'none',
                          background:form.tipo===t?w.orange:w.border,
              ...
              <div style={{position:'relative'}}>
                {lbl('Cliente')}
                <input value={clientSearch} onChange={e=>{setClientSearch(e.target.value);setShowClientDrop(true)}}
                ...
                {showClientDrop && filteredClients.length>0 && (
                  <div style={{position:'absolute',top:'100%',left:0,right:0,zIndex:200,marginTop:4,
                    background:'rgba(255,255,255,0.97)',border:`1px solid ${w.border}`,borderRadius:12,
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
                <div>
                  {lbl('CUIT cliente')}
                  {inp('client_cuit','XX-XXXXXXXX-X')}
                </div>
                <div>
                  {lbl('IVA cliente')}
                  {sel('client_iva',['Responsable Inscripto','Monotributista','Exento','Consumidor Final'])}
                </div>
                <div style={{gridColumn:'1/-1'}}>
                  {lbl('Domicilio cliente')}
                  {inp('client_address','Dirección comercial')}
                </div>
              </div>
              ...
              <div style={{position:'relative'}}>
                ...
                <input value={quoteSearch} onChange={e=>{setQuoteSearch(e.target.value);setShowQuoteDrop(true)}}
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
                <div>
                  {lbl('Condición de venta')}
                  {sel('payment_condition',PAYMENT_METHODS)}
                </div>
            ...
            <>
              ...
              {items.length > 0 && (
                <div style={{display:'flex',flexDirection:'column',gap:8}}>
                  {items.map((it,i)=>(
                    <div key={it.id} style={{display:'grid',gridTemplateColumns:'50px 1fr 70px 90px 70px 70px 80px 30px',
                      ...
                      <input value={it.code||''} onChange={e=>updateItem(i,'code',e.target.value)}
                        placeholder="Cód" style={{...inputStyle,fontSize:11,textAlign:'center'}}/>
                      ...
                      <input type="number" value={it.discount_pct} onChange={e=>updateItem(i,'discount_pct',e.target.value)}
                        placeholder="%" style={{...inputStyle,textAlign:'center'}}/>
                      <select value={it.iva_pct} onChange={e=>updateItem(i,'iva_pct',e.target.value)}
                        style={{...inputStyle}}>
                        {['0','10.5','21','27'].map(v=><option key={v} value={v}>{v}%</option>)}
                      </select>
                      <div style={{fontSize:12,fontWeight:700,color:w.orange,fontFamily:"'Space Mono',monospace",textAlign:'right'}}>
                  ...
                  {/* Header labels */}
                  <div style={{display:'grid',gridTemplateColumns:'50px 1fr 70px 90px 70px 70px 80px 30px',
                    gap:8,padding:'0 14px'}}>
                    {['Código','Descripción','Cant.','P.Unit','Bonif%','IVA','Subtotal',''].map(h=>(
                      <div key={h} style={{fontSize:9,color:w.sub,fontWeight:700,
                        textTransform:'uppercase',letterSpacing:'0.05em',
                        textAlign:['Cant.','P.Unit','Bonif%','Subtotal'].includes(h)?'right':'left'}}>
                        {h}
                      </div>
                    ))}
                  </div>
          ...
          {tab==='pagos' && (
            <>
              {payments.map((p,i)=>(
                <PaymentRow key={p.id} pay={p} onRemove={()=>setPayments(ps=>ps.filter((_,j)=>j!==i))}/>
              ))}
              <div style={{padding:14,borderRadius:14,background:w.orangeLight,border:`1px dashed ${w.orange}44`}}>
                <div style={{fontSize:11,fontWeight:700,color:w.orange,marginBottom:10}}>+ Agregar pago / cobro</div>
                <div style={{display:'grid',gridTemplateColumns:'1fr 120px 130px',gap:8,marginBottom:8}}>
                  ...
                  <input type="number" value={newPay.amount} onChange={e=>setNewPay(p=>({...p,amount:e.target.value}))}
                    placeholder="Monto $" style={{...addInputStyle,textAlign:'right'}}/>
                  <input type="date" value={newPay.due_date} onChange={e=>setNewPay(p=>({...p,due_date:e.target.value}))}
                    style={addInputStyle}/>
                </div>
                <div style={{display:'grid',gridTemplateColumns:'1fr 120px',gap:8,marginBottom:8}}>
                  <input value={newPay.notes} onChange={e=>setNewPay(p=>({...p,notes:e.target.value}))}
                    placeholder="Notas (ej: N° de cheque, banco...)" style={addInputStyle}/>
                  <select value={newPay.status} onChange={e=>setNewPay(p=>({...p,status:e.target.value}))}
                    style={addInputStyle}>
              ...
              {payments.length > 0 && (
                <div style={{padding:'14px 18px',borderRadius:14,
                  ...
                  <div style={{display:'flex',justifyContent:'space-between',marginTop:6}}>
                    <span style={{fontSize:13,color:w.muted}}>Total pagos cargados</span>
                    <span style={{fontSize:13,fontWeight:700,color:w.lime,fontFamily:"'Space Mono',monospace"}}>{fmtMoney(totalPagos)}</span>
                  </div>
                  <div style={{height:1,background:w.border,margin:'10px 0'}}/>
                  <div style={{display:'flex',justifyContent:'space-between'}}>
                    <span style={{fontSize:13,fontWeight:700,color:w.text}}>{diff===0?'✓ Cuadra':diff>0?'Pendiente de cobro':'Excede el total'}</span>
          ...
          {tab==='totales' && (
            <div style={{display:'flex',flexDirection:'column',gap:12}}>
              ...
              <div style={{padding:'20px 24px',borderRadius:18,
                ...
                {[
                  {label:'Importe Neto Gravado',val:form.neto,big:false},
                  {label:'IVA 21%',val:form.iva_21,big:false},
                  {label:'IVA 10.5%',val:form.iva_105,big:false},
                ].map(r=>(
                  <div key={r.label} style={{display:'flex',justifyContent:'space-between',
                    padding:'8px 0',borderBottom:`1px solid ${w.border}`}}>
                    <span style={{fontSize:13,color:w.muted}}>{r.label}</span>
                    <span style={{fontSize:13,fontFamily:"'Space Mono',monospace",color:w.text}}>
                      {fmtMoney(r.val)}
                    </span>
                  </div>
                ))}
                <div style={{display:'flex',justifyContent:'space-between',padding:'8px 0',borderBottom:`1px solid ${w.border}`}}>
                  ...
                  <input type="number" value={form.otros_tributos||''}
                    onChange={e=>set('otros_tributos',e.target.value)}
                    placeholder="0,00"
                ...
                <div style={{display:'flex',justifyContent:'space-between',padding:'14px 0 0'}}>
                  ...
                  <span style={{fontSize:22,fontWeight:900,color:w.orange,fontFamily:"'Space Mono',monospace",
                    textShadow:`0 0 20px ${w.orangeGlow}`}}>
                    {fmtMoney(form.total)}
                  </span>
                </div>
              </div>
              {/* Manual override option */}
              <div style={{padding:'14px 18px',borderRadius:14,background:'rgba(255,255,255,0.6)',
                border:`1px solid ${w.border}`}}>
                <div style={{fontSize:11,color:w.sub,marginBottom:8}}>
                  Si cargás los totales manualmente sin usar la pestaña Productos:
                </div>
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
                  {[
                    {k:'neto',l:'Neto gravado'},
                    {k:'iva_21',l:'IVA 21%'},
                    {k:'iva_105',l:'IVA 10.5%'},
                    {k:'total',l:'Total'},
                  ].map(({k,l})=>(
                    <div key={k}>
                      <div style={{fontSize:11,color:w.sub,marginBottom:3}}>{l}</div>
                      <input type="number" value={form[k]||''}
                        onChange={e=>set(k,Number(e.target.value))}
                        placeholder="0.00"
                        style={{width:'100%',padding:'9px 12px',borderRadius:9,border:`1px solid ${w.border}`,
                          background:'rgba(255,255,255,0.7)',color:w.text,fontSize:13,outline:'none',
...
const inputStyle = {
  padding:'7px 10px',borderRadius:8,border:'1px solid rgba(0,0,0,0.08)',
  background:'rgba(255,255,255,0.8)',color:'#1a1a2e',fontSize:12,outline:'none',
  width:'100%',boxSizing:'border-box',
}
const addInputStyle = {
  padding:'9px 12px',borderRadius:9,border:'1px solid rgba(0,0,0,0.08)',
  background:'rgba(255,255,255,0.8)',color:'#1a1a2e',fontSize:12,outline:'none',
  width:'100%',boxSizing:'border-box',
}
...
export default function Facturacion() {
  ...
  const kpis = useMemo(() => {
    const emitidas = invoices.length
    const totalNeto = invoices.reduce((s,i)=>s+(i.neto||0),0)
    const totalIva = invoices.reduce((s,i)=>s+(i.iva_21||0)+(i.iva_105||0),0)
    const totalFact = invoices.reduce((s,i)=>s+(i.total||0),0)
    const cobradas = invoices.filter(i=>i.status==='COBRADA').length
    const pendientes = invoices.filter(i=>i.status==='PENDIENTE'||i.status==='EMITIDA').length
    const montoPend = invoices.filter(i=>i.status==='PENDIENTE'||i.status==='EMITIDA').reduce((s,i)=>s+(i.total||0),0)
    return { emitidas, totalNeto, totalIva, totalFact, cobradas, pendientes, montoPend }
  }, [invoices])
  ...
  return (
    <div style={{minHeight:'100vh',padding:'24px 28px',background:w.bg,
      ...
      {/* Column headers */}
      {filtered.length > 0 && (
        <div style={{display:'grid',gridTemplateColumns:'80px 1fr 140px 120px 110px 110px 90px',
          ...
          {['N° Factura','Cliente','Fecha','Neto','Total','Pago','Estado'].map(h=>(
            <div key={h} style={{fontSize:10,fontWeight:700,color:w.sub,
              ...
              textAlign:['Neto','Total'].includes(h)?'right':'left'}}>
              {h}
            </div>
          ))}
        </div>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:8}}>
          {filtered.map(inv=>(
            <InvoiceRow key={inv.id} inv={inv}
              onClick={()=>{setSelected(inv);setShowModal(true)}}/>
          ))}
        </div>
      )}
